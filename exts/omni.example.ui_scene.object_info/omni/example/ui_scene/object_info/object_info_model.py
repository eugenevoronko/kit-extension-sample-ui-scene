...
class ObjectInfoModel(sc.AbstractManipulatorModel):
    ...
    def _notice_changed(self, notice: Usd.Notice, stage: Usd.Stage) -> None:
        ...
        for p in notice.GetChangedInfoOnlyPaths():
            if any(path in str(p.GetPrimPath()) for path in self._selected_objects):
    ...
    def get_item(self, identifier, path):
        ...
        if identifier == "material":
            return self._selected_objects[path]["material"]
    ...
    def _on_kit_selection_changed(self):
        ...
        for path in selected_prim_paths:
            ...
            self._selected_objects[path] = {"material": None}
            material, relationship = UsdShade.MaterialBindingAPI(
                prim
            ).ComputeBoundMaterial()
            if material:
                self._selected_objects[path]["material"] = str(
                    material.GetPath())
            else:
                self._selected_objects[path]["material"] = "N/A"

        print("self._selected_objects:", self._selected_objects)
    ...
    def _get_position(self, path):
        ...
        if not stage or (path not in self._selected_objects):
            return [0, 0, 0]
        ...
        self._selected_objects[path] = {"position": None}
        self._selected_objects[path]["position"] = position
