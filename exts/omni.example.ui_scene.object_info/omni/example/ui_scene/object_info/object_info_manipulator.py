...
LINE1_OFFSET = 3
LINE2_OFFSET = 0
UNIT_SELECTION_RADIUS = 50
UNIT_SELECTION_THICKNESS = 2
...
class ObjectInfoManipulator(sc.Manipulator):
    ...
    def on_build(self):
        ...
        if len(self.model._selected_objects) <= 0:
            return

        for path in self.model._selected_objects:
            ...
            # Add object selection indication element
            with sc.Transform(transform=sc.Matrix44.get_translation_matrix(position[0], -position[1], position[2])):
                sc.Arc(UNIT_SELECTION_RADIUS, axis=1,
                       thickness=UNIT_SELECTION_THICKNESS, color=cl.purple, wireframe=True)
